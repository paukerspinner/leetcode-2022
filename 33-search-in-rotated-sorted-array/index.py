...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        while left != right:
            ...
            if target == nums[mid]:
                left = right = mid
            elif (nums[right] < nums[left] <= nums[mid] and (target > nums[mid] or target <= nums[right])) \
            or (nums[left] <= nums[mid] <= nums[right] and target > nums[mid]) \
            or (nums[mid] <= nums[right] < nums[left] and nums[mid] < target <= nums[right]):
                left = mid + 1
            else:
                right = mid
